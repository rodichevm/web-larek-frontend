...
Наследует базовый класс Component<ICard>, отображает компонент карточки товара (в каталоге, предпросмотре и корзине). Отображает поля данные в соответствии с интерфейсом IProduct
